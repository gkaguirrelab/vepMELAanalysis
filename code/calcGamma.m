...
%             [psd(x,y,z,:),freqs]=pwelch(vep',window,[],[],Fs);
            ...
            subplot(1,3,y)
            hold on
            plot(freqs,squeeze(squeeze(psd(x,y,z,:))),['-' C(y)],'LineWidth',L(z))
            ax=gca;ax.XLim=[0 100];ax.Box='off';ax.TickDir='out';ax.YScale='log';ax.YLim=[0.000000001 0.001];
            if z==1
...
[S,F,T]=spectrogram(squeeze(vepM(1,1,:)),100,98,128,Fs);
